...
class MetricsCalculator(PortfolioOptimizer):
    ...
    def portfolioReturnsGraph(self):
        ...
        returns_df = pd.DataFrame(
            {
                ...
                "SP500 Cumulative Return (%)": cumulative_returns_b.values,
        ...
        fig = px.line(
            returns_df,
            x="Date",
            y=["Portfolio Cumulative Return (%)", "SP500 Cumulative Return (%)"],
        ...
        st.markdown(f'**SP500 Returns**: {round(cumulative_returns_b.values[-1], 2)}% ')
        st.plotly_chart(fig)
